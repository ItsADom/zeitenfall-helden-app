...
// Auf `limit` Zeichen kürzen, möglichst an einer Wortgrenze (kein mitten im Wort
// abgeschnittener Rest), Zeilenumbrüche zählen wie Leerzeichen.
...
export function CollapsedText({ text, className, limit = NOTE_LIMIT }: { text: string; className?: string; limit?: number }) {
  const [open, setOpen] = useState(false);
  const print = useDisplayMode() === 'print';
  if (!text) return null;
  // Im Druck (und bei kurzem Text) nichts kürzen.
  if (print || text.length <= limit) return <div className={className}>{text}</div>;
  return (
